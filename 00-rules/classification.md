...
| `scripts/` | **恒久的な自動化プログラム**: 定型処理、データ変換、バッチ | `backup.sh`, `fetch-rss.py` | 実行・作成・保守 |
| `tmp/` | **作業中・一次検証用プログラム**: デバッグ用スクリプト、一時ファイル | `test-search.py`, `temp-scrape.py` | **作成・自由消去** |
...
* **情報源が外部か内部か迷った場合**:
  ...
  * 自分の言葉で要約・整理し直した時点で `02-knowledge/` に新規作成する[cite: 10]。
* **エージェント定義で迷った場合**:
  * 実行手順であれば `.agents/skills/` に寄せ、概念が増えた段階で上記のサブディレクトリへ整理する[cite: 10]。
* **一時ファイル・スクリプトの取り扱い方針**:
  * AIが動作確認や検索・取得用に一時スクリプトを作成する場合は、**必ず `tmp/` 配下に作成**して実行すること。
  * ルート直下（`/`）へ直接スクリプトやテストファイルを配置することは一切禁止する。
  * 使い終わった一時ファイルはタスク完了時に削除するか、`99-trash/` へ退避すること[cite: 10]。
* **削除するか迷った場合**:
  * 物理削除は行わず、必ず `99-trash/` へ移動する[cite: 10]。